...
def home():
    ...
    model_names = [
        file.replace(".pkl", "") for file in model_files if file.endswith(".pkl")
    ]

    default_model_index = (
        model_names.index("xg_boost") if "xg_boost" in model_names else 0
    )

    model_name = st.selectbox(
        "Choose a model to use:", options=model_names, index=default_model_index
    )
    ...
    if st.button("Predict"):
        if user_input and model_name:
            ...
            pred_territory, pred_sector, prediction_t_proba, prediction_s_proba = (
                return_embeedings(
                    user_input,
                    word2vec_model,
                    clf_ter,
    ...
    if uploaded_file is not None:
        ...
        if not all(column in df.columns for column in required_columns):
            st.error(
                f"The uploaded file does not contain all required columns: {', '.join(required_columns)}"
            )
        else:
    ...
    if "uploaded_df" in st.session_state and st.button("Predict from File"):
        processed_df = process_and_predict_file(
            st.session_state["uploaded_df"], model_name
        )
        st.session_state["processed_df"] = (
            processed_df  # Store processed data for potential download
        )
...
def analysis():
    st.title("Análises")
    st.write(
        "Nesta página, é possível observar algumas análises sobre o Território e o Setor. No Território, apresentamos os cinco principais registos de cada país, enquanto no Setor disponibilizamos uma tabela com os registos mais frequentes. Tudo isto é realizado através do token."
    )
    ...
    def suggest_best_models(performance_df, metric_columns):
        ...
        best_models = performance_df.nlargest(3, "Aggregate Score")[
            "Model Name"
        ].tolist()
        return best_models
    ...
    selected_metric = st.selectbox(
        "Select the metric:", options=["Accuracy", "Precision", "Recall"]
    )
    ...
    for column in metric_columns:
        data.append(
            go.Bar(
                x=performance_df["Model Name"], y=performance_df[column], name=column
            )
        )
    ...
    st.write(
        f"Os melhores modelos com base na média entre {metric_name} Territory e {metric_name} Sector são: {', '.join(best_models)}"
    )
    ...
    merged = world.merge(
        territory_df, how="left", left_on="iso_a3", right_on="Território"
    )
    ...
    def click_handler(feature, **kwargs):
        ...
        top5 = territory_df[territory_df["Território"] == iso_a3].nlargest(
            5, "Contagem"
        )
    ...
    folium.GeoJson(
        ...
        style_function=lambda feature: {
            "fillColor": (
                "#0E7A0D"
                if feature["properties"]["Território"]
                in territory_df["Território"].values
                else "#ffffff"
            ),
            "color": "#003300",
        },  # fill color for countries with tokens in top 5
        highlight_function=lambda feature: {"weight": 0},  # remove country highlights
        popup=folium.Popup(
            lambda feature: click_handler(feature, territory_df), max_width=400
        ),  # increase max width of popup text box
    ).add_to(m)
        ...
        if not top5.empty:
            geojson = folium.GeoJson(
                ...
                highlight_function=lambda feature: {
                    "weight": 0
                },  # remove country highlights
                name="geojson",
                popup=folium.Popup(
                    click_handler(feature), max_width=400
                ),  # increase max width of popup text box
            )
        else:
            geojson = folium.GeoJson(
                ...
                style_function=lambda feature: {
                    "fillColor": "#ffffff",
                    "color": "#000000",
                },  # remove fill color for countries without list
                highlight_function=lambda feature: {
                    "weight": 0
                },  # remove country highlights
                name="geojson",
    ...
    sector_df1 = sector_df.drop(
        ["Unnamed: 0", "Descrição do Setor Institucional"], axis=1
    )
    ...
    def update_records(selected_sector):
        st.write(
            f"Resgitos do {selected_sector} ({sector_df.loc[sector_df['Setor Institucional'] == selected_sector, 'Descrição do Setor Institucional'].iloc[0]}):"
        )
...
def training():
    ...
    model_name = st.selectbox(
        "Choose a model to retrain:", options=model_names, index=default_model_index
    )

    # File upload for bulk data
    st.header("Upload a File for Retraining")
    bulk_uploaded_file = st.file_uploader(
        "Upload your file (CSV or Excel) containing multiple instances",
        type=["csv", "xlsx"],
        key="bulk-file-uploader",
    )

    required_columns = [
        "DescricaoInstrumento",
        "SetorInstitucionalCon",
        "TerritorioCon",
    ]
    ...
    with st.form(key="training_form"):
        ...
        MaturidadeOriginal = st.selectbox(
            "MaturidadeOriginal", ["01", "10", "06", "07", "08", "_Z"]
        )
        SetorInstitucionalCon = st.selectbox(
            "SetorInstitucionalCon",
            [
                "S11",
                "S121",
    ...
    if submit_button or "bulk_uploaded_df" in st.session_state:
        ...
        backup_data_path = (
            f"projetofinal/data_train/backup/{current_datetime}_data_train.csv"
        )
        ...
        # Preprocessing
        updated_data["tokenized_Descricao_text"] = updated_data[
            "DescricaoInstrumento"
        ].apply(lambda x: simple_preprocess(x))
        word2vec_model = Word2Vec(
            sentences=updated_data["tokenized_Descricao_text"],
            vector_size=100,
            window=5,
            min_count=1,
            workers=4,
        ...
        updated_data["avg_embedding"] = updated_data["tokenized_Descricao_text"].apply(
            lambda tokens: compute_avg_embedding(tokens, word2vec_model)
        )
        X = updated_data["avg_embedding"].apply(pd.Series).to_numpy()

        unique_categories_ter = updated_data["TerritorioCon"].unique()
        category_mapping_ter = dict(
            zip(unique_categories_ter, range(len(unique_categories_ter)))
        )
        inverted_mapping_ter = {
            value: key for key, value in category_mapping_ter.items()
        }

        unique_categories_sec = updated_data["SetorInstitucionalCon"].unique()
        category_mapping_sec = dict(
            zip(unique_categories_sec, range(len(unique_categories_sec)))
        )
        inverted_mapping_sec = {
            value: key for key, value in category_mapping_sec.items()
        }

        updated_data["encoded_label_territorio"] = updated_data["TerritorioCon"].apply(
            encode_target, args=[category_mapping_ter]
        )
        updated_data["encoded_label_setor"] = updated_data[
            "SetorInstitucionalCon"
        ].apply(encode_target, args=[category_mapping_sec])
        ...
        X_train_1, X_test_1, y_train_1, y_test_1 = train_test_split(
            X, y1, test_size=0.1, random_state=41
        )
        X_train_2, X_test_2, y_train_2, y_test_2 = train_test_split(
            X, y2, test_size=0.1, random_state=41
        )
        ...
        if model_name in performance_df["Model Name"].values:
            performance_df.update(
                performance_df.loc[
                    performance_df["Model Name"] == model_name,
                    new_performance_row.keys(),
                ].assign(**new_performance_row)
            )
        else:
            performance_df = performance_df.append(
                new_performance_row, ignore_index=True
            )
...
selection = st.sidebar.radio(
    "Go to", [f"{pages[key]['icon']} {key}" for key in pages.keys()], index=0
)
