...
def map_dict(df_clean):
    ...
    df_clean["encoded_label_territorio"] = df_clean["TerritorioCon"].apply(
        encode_target, args=[category_mapping_ter]
    )
    df_clean["encoded_label_setor"] = df_clean["SetorInstitucionalCon"].apply(
        encode_target, args=[category_mapping_sec]
    )
...
def compute_avg_embedding(tokens, word2vec_model):
    ...
    if embeddings:  # Embeddings found
        return np.array(embeddings).mean(
            axis=0
        )  # Return average embedding as a NumPy array
...
def embbeded_model(df_clean):
    ...
    """
    df_clean["tokenized_Descricao_text"] = df_clean["DescricaoInstrumento"].apply(
        lambda x: simple_preprocess(x)
    )
    word2vec_model = Word2Vec(
        sentences=df_clean["tokenized_Descricao_text"],
        vector_size=100,
        window=5,
        min_count=1,
        workers=4,
...
def embedded_data(df_clean, words2vec_model):
    ...
    """
    df_clean["avg_embedding"] = df_clean["tokenized_Descricao_text"].apply(
        compute_avg_embedding, args=(words2vec_model,)
    )
